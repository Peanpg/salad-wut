...
function doPost(e) {
  ...
  try {
    ...
    if (payload.action === 'deleteLotAssets') {
      return json_(deleteLotAssets_(payload));
    }
    if (payload.action === 'deleteDailyLogAssets') {
      return json_(deleteDailyLogAssets_(payload));
    }
...
function deleteLotAssets_(payload) {
  const root = DriveApp.getFolderById(LOT_ROOT_FOLDER_ID);
  const folderName = sanitizeName_(payload.lotFolderName || '');
  let trashedFolders = 0;
  let trashedFiles = trashFilesFromUrls_(payload.photoUrls || []);
...
function deleteDailyLogAssets_(payload) {
  const root = DriveApp.getFolderById(DAILY_ROOT_FOLDER_ID);
  const folderName = sanitizeName_(payload.dateFolderName || '');
  const trashedFiles = trashFilesFromUrls_(payload.photoUrls || []);
  let trashedEmptyFolders = 0;
  ...
  return {
    ...
    trashedFiles: trashedFiles,
    trashedEmptyFolders: trashedEmptyFolders
  };
}

function trashFilesFromUrls_(urls) {
  ...
  urls.forEach(function(url) {
    ...
    try {
      ...
      console.log('ไม่สามารถลบไฟล์ ' + id + ': ' + errorMessage_(error));
    }
  });
  return count;
}
